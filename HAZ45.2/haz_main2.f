...
c     Read Run File
     ...
     2               psCorFlag, damp, iBoundFlag, minlat, maxlat, minlong, maxlong, distmax,
     ...
     6               coefcountRrup, coefcountRjb, iMixture, version, boundfilein )
...
!      Read the boundary file, if Flag is 1.  
!      The boundary polygon should be convex and defined CCW
       if (iBoundFlag .eq. 1)  then

         call read_tvfz_bound (boundfilein,nvert,xPOL,yPOL)
!      Check to see if the site is within the boundary
         call PNPOLY (SiteX,SiteY,xPOL,yPOL,nvert,INOUTSTA)
         I0=INOUTSTA

c       Convert Coordinates for the boundary file to x, y in km 
         call ConvertCoordinatesBnd ( siteX, siteY,
     1         yPOL, xPOL, grid_xPOL, grid_yPOL, nvert )
...
c             Call PNPOLY to see if site and rupture point are in the bounded area   
c             if they are TVZ_DIST = Rrup, if they are out call segment
c             3d and calculate the traveled distance in the bounded area 
              if (iBoundFlag .eq. 1 ) then 

c !!!  The iLocX and iLocY give only one point for the largest magnitudes. 
c !!!  The TVZ_DIST is computed for each one of the fltgrid_x and fltgrid_y 
c !!!  and not for the closest point of the site. 
c !!!  It should be added in the CalcDist subroutine in the future

                TVZ_DIST=distRup
                if (sourceType(iFlt) .eq. 1 .or. sourceType(iFlt) .eq. 2 ) then
                   temp_x=fltgrid_x(iLocY,iLocX)
                   temp_y=fltgrid_y(iLocY,iLocX)
                   CALL PNPOLY (temp_x,temp_y,grid_xPOL,grid_yPOL,nvert,INOUTSTA)
                   I1=INOUTSTA
c                   write(*,*) I1,temp_x,temp_y, 'Case1'
                   if (I0 .le. 0 .and. I1 .le. 0) then
                     call segment3d (nvert,hypoDepth,x0,y0,temp_x,temp_y,
     &                          distepi,grid_xPOL,grid_yPOL,TVZ_DIST,bFLAG)
                   endif
                   if (I0*I1 .lt. 0 ) then
c    We need the distance of the point inside the boundary to the point of intersect 
c    if the site is out of the boundary.          
                     if (I1 .gt. 0)  call seginout (nvert,hypoDepth,x0,y0,temp_x,temp_y,
     &                          distepi,grid_xPOL,grid_yPOL,TVZ_DIST,bFLAG)
c    if the site is in the boundary                                   
                     if (I0 .gt. 0)  call seginout (nvert,hypoDepth,temp_x,temp_y,x0,y0,
     &                          distepi,grid_xPOL,grid_yPOL,TVZ_DIST,bFLAG)
                   endif
                elseif (sourceType(iFlt) .eq. 3 .or. sourceType(iFlt) .eq. 4) then 
                   temp_x=grid_x(iLocX) 
                   temp_y=grid_y(iLocX)
                   CALL PNPOLY (temp_x,temp_y,grid_xPOL,grid_yPOL,nvert,INOUTSTA)
                   I1=INOUTSTA
                   if (I0 .le. 0 .and. I1 .le. 0) then
                     call segment3d (nvert,hypoDepth,x0,y0,temp_x,temp_y,
     &                               distepi,grid_xPOL,grid_yPOL,TVZ_DIST,bFLAG)
c                   write(*,*) I1,temp_x,temp_y, 'Case 2', bFLAG
                   endif
                   if (I0*I1 .lt. 0 ) then 
c    if the site is out of the boundary                                   
                     if (I1 .gt. 0)  call seginout (nvert,hypoDepth,x0,y0,temp_x,temp_y,
...
c              write(*,*) 'TVZ_DIST=',TVZ_DIST, 'Rrup=', distrup
...
c              Compute the median and sigma of the ground motions
     ...
     4               distepi, disthypo, depthvs10, depthvs15, D25, tau,
     5               zTOR, theta_site, RupWidth, vs30_class, forearc, Rx, phi,
     6               cfcoefrrup, cfcoefrjb, Ry0, TVZ_DIST )
               elseif (sourceType(iFlt) .eq. 7) then
                 call meanInten ( distRup, distJB, distSeismo,
     1               HWFlag, mag, jcalc1, specT(iProb),  
...
C AAS 01/16/2016 Adding NGA-West2 damping scaling factors
              if(damp. ne. 5.) then
                 lgInten = lgInten + DampingFactor(damp,specT(iProb),mag,distRup)
              endif
...
c               Loop over synchronous ruptures (aleatory)          
     ...
     5                distepi, disthypo, depthvs10, depthvs15, D25, tau,
     6                syn_zTOR(iFlt,isyn), theta_site, syn_RupWidth(iFlt,isyn), 
     7                vs30_class, forearc, syn_Rx(iFlt,isyn), phi,
     8                cfcoefrrup, cfcoefrjb, syn_Ry0(iFlt,isyn), TVZ_DIST )
